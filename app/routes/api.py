...
def dwell():
    ...
    col = db.dwell_time_summary

    # ── Daily trend by dwell bucket ──
    daily_pipeline = [
        {"$match": {**str_date_filter(date_from, date_to_ex), **hour_expr_str(hour_from, hour_to), "store_code": store_code}},
        {"$group": {
    ...
    rows = list(col.aggregate(daily_pipeline))
    ...
    # ── Hourly pattern (averaged across all selected dates) ──
    hourly_pipeline = [
        {"$match": {**str_date_filter(date_from, date_to_ex), **hour_expr_str(hour_from, hour_to), "store_code": store_code}},
        {"$group": {
            "_id":   {"$substr": ["$date_time", 11, 2]},
    ...
    hourly_rows = list(col.aggregate(hourly_pipeline))
    ...
    for r in hourly_rows:
        n = len(r.get("dates", [1])) or 1
        hourly_labels.append(f"{r['_id']}:00")
        hourly_lt2.append(round(r["lt2"] / n))
        hourly_b2_10.append(round(r["b2_10"] / n))
        hourly_gt10.append(round(r["gt10"] / n))

    total_lt2 = sum(lt2)
    total_b2_10 = sum(b2_10)
    total_gt10 = sum(gt10)
    grand_total = total_lt2 + total_b2_10 + total_gt10
    ...
    return jsonify({
        ...
        "gt10":         gt10,
        "total_lt2":    total_lt2,
        "total_b2_10":  total_b2_10,
        "total_gt10":   total_gt10,
        "grand_total":  grand_total,
        "hourly_labels":  hourly_labels,
        "hourly_lt2":     hourly_lt2,
        "hourly_b2_10":   hourly_b2_10,
        "hourly_gt10":    hourly_gt10,
        "date_from":      date_from,
        "date_to":        date_to,
    })
